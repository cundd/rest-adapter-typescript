...
it('new', () => {
    ...
    hosts.forEach(host => {
        protocols.forEach(protocol => {
            ports.forEach(port => {
                paths.forEach(path => {
                    const configuration = new AdapterConfiguration(host, protocol, port, path);
...
it('new should throw for invalid hostname', () => {
    ...
    hosts.forEach((host) => {
        expect(() => {
            new AdapterConfiguration(host as any);
...
it('new should throw for invalid protocol (empty protocol)', () => {
    ...
    protocols.forEach((protocol) => {
        expect(() => {
            new AdapterConfiguration('localhost', protocol as any);
...
it('new should throw for invalid protocol (missing colon)', () => {
    ...
    protocols.forEach((protocol) => {
        expect(() => {
            new AdapterConfiguration('localhost', protocol);
...
it('new should throw for invalid port', () => {
    ...
    ports.forEach((port) => {
        expect(() => {
            new AdapterConfiguration('localhost', undefined, port as any);
        ...
        protocols.forEach(protocol => {
            ports.forEach(port => {
                paths.forEach(path => {
                    ...
                    const configuration = AdapterConfiguration.fromUrl(url);
